...
export default function AddOrEditRun({ returnToRuns, editRun }) {
  ...
  const [logDate, setLogDate] = useState(
    (editing ? new Date(editRun.loggedAt) : new Date()).toISOString().split("T")[0]
  )
  ...
  function setParsedInt(fn, value) {
    ...
    fn(value === "" || Number.isNaN(value) ? fallback : parseInt(value, 10))
  }

  const handleAddOrEditRun = useCallback(() => {
    setError("")
    ...
    if (speed !== "" && !Number.isNaN(speed)) payload.speed = speed
    if (timeHH !== "" && !Number.isNaN(timeHH)) payload.timeHH = timeHH
    if (timeMM !== "" && !Number.isNaN(timeMM)) payload.timeMM = timeMM
    if (timeSS !== "" && !Number.isNaN(timeSS)) payload.timeSS = timeSS
    payload.logDate = logDate

    dispatch({ type, payload: buildRunPayload(payload) })
    return returnToRuns()
  }, [dispatch, editRun, editing, logDate, returnToRuns, speed, timeHH, timeSS, timeMM])
  ...
  return (
    <div className="container">
      ...
      <div className="AddOrEditRun-input-container">
        ...
        <input
          value={logDate}
          onChange={({ target: { value } }) => setLogDate(value)}
          type="date"
          className="AddOrEditRun-input"
          id="logDate"
        />
      </div>
      ...
      <div className="AddOrEditRun-input-container">
        ...
        <input
          value={speed}
          onChange={({ target: { value } }) => setParsedInt(setSpeed, value)}
          type="number"
          className="AddOrEditRun-input"
          id="speed"
          placeholder="enter treadmill speed"
        />
      </div>
      ...
      <div className="AddOrEditRun-input-container">
        ...
        <div className="AddOrEditRun-time-container">
          <input
            value={timeHH}
            onChange={({ target: { value } }) => setParsedInt(setTimeHH, value)}
            className="AddOrEditRun-time"
            id="time"
            type="number"
            placeholder="HH"
          />
          ...
          <input
            value={timeMM}
            onChange={({ target: { value } }) => setParsedInt(setTimeMM, value)}
            className="AddOrEditRun-time"
            type="number"
            placeholder="MM"
          />
          ...
          <input
            value={timeSS}
            onChange={({ target: { value } }) => setParsedInt(setTimeSS, value)}
            className="AddOrEditRun-time"
            type="number"
            placeholder="SS"
          />
      ...
      <Button handleClick={handleAddOrEditRun}>
        {editing ? (
          ...
          <>
            <AiOutlinePlus className="AddOrEditRun-icon" />
            Add new run
          </>
        )}
      </Button>
      ...
      {editing && !showDeleteConfirm && (
        <Button handleClick={handleDelete} type="danger">
          Delete run
        </Button>
      )}

      {showDeleteConfirm && (
        <Button handleClick={handleConfirmedDelete} type="danger">
          Are you sure?
        </Button>
      )}
